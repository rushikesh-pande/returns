package com.order.returns.db.config;
 ...
 *
 * Usage in service:
 *   @Cacheable(value = "returnsCache", key = "#id")
 *   @CacheEvict(value = "returnsCache", key = "#id")
 *   @CachePut(value  = "returnsCache", key = "#result.id")
 */
@Configuration
@EnableCaching
public class CacheConfig {
    ...
    public CacheManager cacheManager(RedisConnectionFactory factory) {
        ...
        // ── returns specific caches ─────────────────────────────────────
        caches.put("returnsCache",       defaultConfig().entryTtl(MEDIUM));
        caches.put("returnsListCache",   defaultConfig().entryTtl(SHORT));
        caches.put("returnsCountCache",  defaultConfig().entryTtl(SHORT));
        caches.put("returnsRefCache",    defaultConfig().entryTtl(LONG));
